...
from storm.qlib_adapter import apply_qlib_like_processors, build_windowed_qlib_dataframe, calc_prediction_metrics, build_prediction_payload_from_frame
...
def _build_dataset(config):
    ...
    raw_df = build_windowed_qlib_dataframe(
        ...
        include_asset_identity=getattr(config, "include_asset_identity", False),
    )
...
def main(args):
    ...
    logger.info(f"| Raw Qlib windowed dataframe shape: {raw_qlib_df.shape}")
    logger.info(f"| Model Qlib windowed dataframe shape: {model_qlib_df.shape}")
    logger.info(
