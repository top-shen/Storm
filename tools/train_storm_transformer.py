...
def _build_dataloaders(config):
    ...
    train_loader = DataLoader(train_dataset, batch_size=config.batch_size, shuffle=True, drop_last=True, pin_memory=True, num_workers=config.num_workers, collate_fn=collate_fn)
    train_eval_loader = DataLoader(train_dataset, batch_size=config.batch_size, shuffle=False, drop_last=False, pin_memory=True, num_workers=config.num_workers, collate_fn=collate_fn)
    valid_loader = DataLoader(valid_dataset, batch_size=config.batch_size, shuffle=False, drop_last=False, pin_memory=True, num_workers=config.num_workers, collate_fn=collate_fn)
    test_loader = DataLoader(test_dataset, batch_size=config.batch_size, shuffle=False, drop_last=False, pin_memory=True, num_workers=config.num_workers, collate_fn=collate_fn)
...
def _build_metric_inputs(end_timestamps: torch.Tensor, assets: List[List[str]], pred: torch.Tensor, target: torch.Tensor, label_column: str):
    ...
    index = pd.MultiIndex.from_arrays([pd.to_datetime(end_timestamp_values), asset_values], names=["datetime", "instrument"])
...
def _evaluate(model, dataloader, device, label_column, save_path=None):
    ...
    all_label_frames = []
    all_pred_series = []
    payload = {"end_timestamp": [], "asset": [], "pred_label": [], "true_label": []}
    ...
    with torch.no_grad():
        for batch in dataloader:
            ...
            label_df, pred_series, batch_payload = _build_metric_inputs(batch["history"]["end_timestamp"], batch["asset"], pred, target, label_column)
...
def main(args):
    ...
    model = StormTransformer(
        ...
        max_len=config.history_timestamps,
    ).to(device)
    ...
    if args.train:
        ...
        for epoch in range(1, config.num_epochs + 1):
            train_loss = _run_epoch(model, train_loader, optimizer, device, config.grad_clip)
            valid_metrics = _evaluate(model, valid_loader, device, config.label_column)
            log_item = {
                "epoch": epoch,
            ...
            if valid_metrics["MSE"] < best_valid_mse:
                ...
                logger.info(f"| Saved best STORM Transformer model: {model_path}")
            else:
                patience += 1
    ...
    if args.test:
        ...
        if not os.path.exists(ckpt):
            raise FileNotFoundError(f"STORM Transformer checkpoint not found: {ckpt}")
        _load_checkpoint(model, optimizer=None, checkpoint_path=ckpt, device=device)
        logger.info(f"| Load STORM Transformer model: {ckpt}")
        test_stats = {}
        for split, loader in [("train", train_eval_loader), ("valid", valid_loader), ("test", test_loader)]:
            metrics = _evaluate(model, loader, device, config.label_column, save_path=os.path.join(config.exp_path, f"{split}_predictions.joblib"))
